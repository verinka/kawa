  ...
  function caption() {
      return (<>
        <span dangerouslySetInnerHTML={{__html: item.text}}></span><br/>
            <p>Weight: {item.weight} г.</p>
            {
              item.pid === '3' || item.pid === '4'
                ? '<p>Quantity: {item.quantity} it.</p>'
                : ''
            }
            ...
            }
            {
              item.proteins && item.fats && item.carbohydrates
                ? '<p>Nutritional value:<br>&emsp;proteins — {item.proteins} г.<br>&emsp;fats — {item.fats} г.<br>&emsp;carbohydrates — {item.carbohydrates} г.</p>'
                : ''
            }
            {
              item.shelf_life
                ? '<p>Shelf life: ' + item.shelf_life + ' month</p>'
                : ''
            }
            {(() => {
